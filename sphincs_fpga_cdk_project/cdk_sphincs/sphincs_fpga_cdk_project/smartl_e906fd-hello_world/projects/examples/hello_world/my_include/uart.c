...
//{
//    volatile uint32_t timeout = 500000UL;
//
//    /* Poll LSR Data Ready bit */
//    while (!((*(reg8_t *)(uart_device->register_map + CK_UART_LSR)) & CK_LSR_DR));
//	{
//        if (--timeout == 0)
//            return 1;   /* timeout */
//    }
//    *c = *(reg8_t *)(uart_device->register_map + CK_UART_RBR);
//    return 0;
//}

uint32_t ck_uart_getc(p_ck_uart_device uart_device, uint8_t *c)
{
    volatile uint32_t timeout = 5000000UL;
    uint8_t lsr;

    while (1) {
        lsr = *(reg8_t*)(uart_device->register_map + CK_UART_LSR);
        if (lsr & CK_LSR_DR) {
            *c = *(reg8_t*)(uart_device->register_map + CK_UART_RBR);
            return 0;
        }
        if (lsr & CK_LSR_OE) {   // 溢出错误
            // 先读走 RBR 清除标志（虽然可能数据已丢）
            (void)*(reg8_t*)(uart_device->register_map + CK_UART_RBR);
            return 2;   // 自定义：溢出错误
        }
        if (--timeout == 0)
            return 1;
